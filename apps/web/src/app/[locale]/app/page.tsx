...
export default async function AppDashboardPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const publishedQuizzes = mockQuizzes.filter((quiz) => quiz.state === 'published');
  return <div className="mx-auto w-full max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
    ...
    <div className="grid gap-4 sm:grid-cols-3"><Metric icon={<BookOpenCheck className="size-5" />} label={t('app.metrics.resources')} value="12" /><Metric icon={<Flame className="size-5" />} label={t('app.metrics.streak')} value={t('app.metrics.streakValue')} /><Metric icon={<Trophy className="size-5" />} label={t('app.metrics.points')} value="185" /></div>
    ...
    <section className="mt-8"><div className="mb-4 flex items-center justify-between"><h2 className="text-xl font-semibold text-on-background">{t('app.quizzesTitle')}</h2><span className="text-sm text-on-surface-variant">{t('app.rewards')}</span></div><div className="grid gap-4 lg:grid-cols-2">{publishedQuizzes.map((quiz) => <Card key={quiz.id} className="flex flex-wrap items-center justify-between gap-4 p-5"><div><Badge tone="reward">{quiz.examPhase}</Badge><h3 className="mt-3 font-semibold text-on-surface">{quiz.title}</h3><p className="mt-1 text-sm text-on-surface-variant">{quiz.questionCount} questions · {quiz.totalPoints} points</p></div><LinkButton href={`/${locale}/app/quiz/${quiz.id}`} variant="reward" size="sm">{t('app.takeQuiz')}<ArrowRight className="ms-2 size-4" /></LinkButton></Card>)}</div></section>
