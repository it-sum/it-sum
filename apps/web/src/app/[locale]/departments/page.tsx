      ...
      <div className="mx-auto grid max-w-[var(--it-sum-content-max-width)] gap-8 px-4 py-12 sm:px-6 lg:grid-cols-[0.8fr_1.2fr] lg:px-8">
        <aside>
          ...
          <div className="mt-4 space-y-3">
            {mockDepartments.map((department, index) => (
              <Card key={department.id} variant={index === 0 ? 'elevated' : 'outlined'} className={index === 0 ? 'border-2 border-primary/30' : ''}>
                <CardBody className="flex items-center gap-3 p-4">
                  ...
                  <div className="min-w-0 flex-1">
                    <h3 className="text-title-small text-on-surface">{department.name[locale === 'ar' ? 'ar' : 'en']}</h3>
        ...
        <section aria-labelledby="it-department-title">
          <div className="flex items-center justify-between gap-3">
            <div>
              <h2 id="it-department-title" className="text-headline-small text-on-surface">{mockDepartmentTree.name[locale === 'ar' ? 'ar' : 'en']}</h2>
              <p className="mt-1 text-body-medium text-on-surface-variant">{mockDepartmentTree.description[locale === 'ar' ? 'ar' : 'en']}</p>
            </div>
            ...
            {mockDepartmentTree.batches.map((batch) => (
              <Card key={batch.id} variant="outlined">
                <CardHeader className="flex-row items-center justify-between gap-3">
                  <div className="flex items-center gap-3"><div className="grid size-9 place-items-center rounded-full bg-secondary-container text-on-secondary-container" data-numeric="true">{batch.level}</div><CardTitle>{batch.name[locale === 'ar' ? 'ar' : 'en']}</CardTitle></div>
                    ...
                    batch.semesters.map((semester) => (
                      <div key={semester.id} className="rounded-xl bg-surface-container px-4 py-4">
                        <div className="flex flex-wrap items-center justify-between gap-2">
                          <div><h3 className="text-title-small text-on-surface">{semester.name[locale === 'ar' ? 'ar' : 'en']}</h3><p className="mt-1 text-body-small text-on-surface-variant">{t('departments.resourcesCount', { count: semester.resourceCount ?? 0 })}</p></div>
                        ...
                        {semester.courses.length > 0 && <div className="mt-3 grid gap-2 sm:grid-cols-2">
                          {semester.courses.map((course) => (
                            <Link key={course.id} href={`/resources?course=${course.id}` as never} className="state-layer flex items-center gap-3 rounded-lg border border-outline-variant/60 bg-surface p-3 hover:border-primary">
                              ...
                              <div className="min-w-0 flex-1"><p className="truncate text-title-small text-on-surface">{course.name[locale === 'ar' ? 'ar' : 'en']}</p><p className="mt-0.5 text-body-small text-on-surface-variant">{course.code} · {course.resourceCount} {t('resources.pages')}</p></div>
