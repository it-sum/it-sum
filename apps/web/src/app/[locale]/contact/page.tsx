  ...
  return (
    <>
      ...
      <main className="relative overflow-hidden bg-surface-container-lowest">
        ...
        <div className="relative mx-auto max-w-[var(--it-sum-content-max-width)] px-4 py-12 sm:px-6 md:py-20 lg:px-8">
          <div className="grid gap-10 lg:grid-cols-[0.72fr_1.28fr] lg:gap-16">
            <aside>
              ...
              <div className="mt-8 space-y-3">
                <Card variant="filled" className="glass-surface">
                  <CardBody className="flex gap-4 p-5">
                    <div className="grid size-11 shrink-0 place-items-center rounded-[15px] bg-primary text-on-primary"><MessageCircle className="size-5" aria-hidden="true" /></div>
                    <div><h3 className="text-title-small text-on-surface">{t('contact.helperTitle')}</h3><p className="mt-1 text-body-medium text-on-surface-variant">{t('contact.responseTime')}</p></div>
                  </CardBody>
                </Card>
                <Card variant="outlined" className="glass-control">
                  <CardBody className="flex items-center gap-4 p-5">
                    <div className="grid size-11 shrink-0 place-items-center rounded-[15px] bg-primary-container/75 text-on-primary-container"><Mail className="size-5" aria-hidden="true" /></div>
                    <div><p className="text-label-large text-on-surface-variant">{t('contact.emailLabel')}</p><a href="mailto:support@it-sum.org" className="mt-1 block text-body-medium font-semibold text-primary underline-offset-4 hover:underline">support@it-sum.org</a></div>
                  </CardBody>
                </Card>
                <Card variant="outlined" className="glass-control">
                  <CardBody className="flex gap-4 p-5">
                    <Clock3 className="mt-0.5 size-5 shrink-0 text-tertiary" aria-hidden="true" />
                    <div><p className="text-label-large text-on-surface-variant">{t('contact.responseLabel')}</p><p className="mt-1 text-body-medium font-semibold text-on-surface">{t('contact.responseTime')}</p></div>
                  </CardBody>
                </Card>
              </div>
              <div className="glass-soft mt-8 flex items-start gap-3 rounded-[15px] border border-outline-variant/60 p-4"><ShieldCheck className="mt-0.5 size-5 shrink-0 text-primary" aria-hidden="true" /><p className="text-body-small text-on-surface-variant">{t('contact.secureNote')}</p></div>
            </aside>
